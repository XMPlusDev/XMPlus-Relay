...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/XMPlusDev/XMPlus-Relay/install/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/XMPlusDev/XMPlus-Relay/install/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XMPlus -N --no-check-certificate https://raw.githubusercontent.com/XMPlusDev/XMPlus-Relay/install/XMPlus.sh
    if [[ $? != 0 ]]; then
        echo ""
